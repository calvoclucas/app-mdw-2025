...
const genericRestaurantImage = "https://images.pexels.com/photos/262978/pexels-photo-262978.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2";
...
const EmpresaCard: React.FC<{ empresa: EmpresaConUsuario; imagen: string; onFavorite?: (id: string) => void; favorito?: boolean }> = ({ empresa, imagen, onFavorite, favorito }) => {
  const navigate = useNavigate();

  const abiertoAhora = (() => {
    if (!empresa.empresa || !empresa.empresa.horario_apertura || !empresa.empresa.horario_cierre) return false;
    ...
    return apertura > cierre ? horaActual >= apertura || horaActual < cierre : horaActual >= apertura && horaActual < cierre;
  })();
    ...
    <div className="group bg-white rounded-xl shadow-sm overflow-hidden transform transition duration-300 hover:scale-[1.03] hover:shadow-md border border-gray-100 m-4 cursor-pointer relative">
      <div className="relative">
        <img
          ...
          onError={(e) => { e.currentTarget.src = genericRestaurantImage; }}
        />
        <div className={`absolute top-2 left-2 px-2 py-1 text-xs font-bold rounded-full shadow-md text-white`} style={{ backgroundColor: abiertoAhora ? "#16a34a" : "#dc2626" }}>
          {abiertoAhora ? `ABIERTO | Cierra ${empresa.empresa?.horario_cierre}` : `CERRADO | Abre ${empresa.empresa?.horario_apertura}`}
        </div>
        ...
        {onFavorite && (
          <button
            className={`absolute top-2 right-2 text-lg p-1 rounded-full ${favorito ? "text-yellow-400" : "text-gray-300"} hover:text-yellow-500 transition`}
            onClick={(e) => { e.stopPropagation(); onFavorite(empresa._id); }}
          >
            ★
          </button>
        )}
      ...
      <div className="p-4 flex flex-col gap-2 text-sm">
        ...
        {empresa.empresa?.telefono && <p className="text-gray-600">{empresa.empresa.telefono}</p>}
      </div>
    </div>
  );
};
...
const Dashboard: React.FC = () => {
    ...
    const fetchEmpresas = async () => {
      ...
      try {
        if (user.role === "cliente") {
          const res = await axios.get<EmpresaConUsuario[]>(`${API_URL}/Api/GetEmpresasConUsuario`);
  ...
  const empresasFiltradas = empresas.filter((e) => {
    const nombreCoincide = e.empresa?.nombre?.toLowerCase().includes(searchTerm.toLowerCase());
    const productosCoinciden = e.productos?.some(p => p.nombre.toLowerCase().includes(searchTerm.toLowerCase()));

    let categoriaCoincide = filterCategory ? e.empresa?.categoria === filterCategory : true;

    let openCoincide = true;
    if (filterOpen) {
      const horaActual = new Date().getHours();
      const apertura = parseInt(e.empresa?.horario_apertura?.split(":")[0] || "0");
      const cierre = parseInt(e.empresa?.horario_cierre?.split(":")[0] || "24");
      const abierto = apertura > cierre ? horaActual >= apertura || horaActual < cierre : horaActual >= apertura && horaActual < cierre;
      openCoincide = (filterOpen === "open" && abierto) || (filterOpen === "closed" && !abierto);
    }

    return (nombreCoincide || productosCoinciden) && categoriaCoincide && openCoincide;
  });
    ...
    <div className="min-h-screen flex flex-col justify-between bg-gradient-to-br from-orange-50 via-yellow-50 to-amber-50">
      <header className="bg-white shadow-sm flex items-center justify-between px-4 py-3 sticky top-0 z-50 border-b border-gray-100">
        ...
        <div className="flex items-center gap-3 relative">
          <button
            ...
            onClick={() => {
              const id = user?.role === "empresa" ? user?.empresa?._id : user?.cliente?._id;
          ...
          <div className="relative">
            ...
            {profileOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden z-50">
                <button className="block w-full text-left px-4 py-2 hover:bg-yellow-50 text-gray-700" onClick={() => navigate("/profile")}>Mi perfil</button>
                {user?.name !== "Invitado" ? (
                  <button className="block w-full text-left px-4 py-2 hover:bg-yellow-50 text-gray-700" onClick={handleLogout}>Cerrar sesión</button>
                ) : (
                  <button className="block w-full text-left px-4 py-2 hover:bg-yellow-50 text-gray-700" onClick={handleLogin}>Iniciar sesión</button>
                )}
              </div>
            )}
          </div>
      ...
      <main className="px-4 sm:px-6 py-6 flex-1">
        <h2 className="text-lg md:text-xl font-medium text-gray-800 mb-4">
          Bienvenido, <span className="text-yellow-400 font-semibold">{`${user?.name || ""} ${user?.lastName || ""}`.trim() || user?.email || "Invitado"}</span>
        </h2>
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:gap-4">
          ...
          <div className="flex flex-col md:flex-row gap-2 mt-2 md:mt-0">
            <select className="p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400 transition" onChange={(e) => setFilterCategory(e.target.value)}>
            ...
            <select className="p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400 transition" onChange={(e) => setFilterOpen(e.target.value)}>
        ...
        {empresasFavoritas.length > 0 && (
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Tus Favoritos</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {!loading && empresasFiltradas.map((item) => (
            <EmpresaCard
              key={item._id}
              empresa={item}
      ...
      {showGuestToast && (
        <div className="fixed top-24 right-10 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded shadow-lg flex items-start gap-3 w-80 animate-slide-in z-50">
          <svg className="w-6 h-6 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-700">Para ver tus pedidos y obtener una mejor experiencia, inicia sesión en tu cuenta.</p>
          </div>
        </div>
      )}
